...
# -*- coding: utf-8 -*-
"""
Script to rename all note files in the notes directory to English filenames
...
def main():
    ...
    print(f"📁 Found {len(list(notes_dir.glob('*.html')))} HTML files in notes directory")
    ...
    for old_file in html_files:
        ...
        for i in range(len(base_name), -1, -1):
            if base_name[i].isdigit() or base_name[i] == ' ':
                clean_title = base_name[:i]
            else:
                break
